...
func (h *UserHandler) CreateUser(c *fiber.Ctx) error {
	...
	if err := c.BodyParser(&req); err != nil {
		...
		return h.sendErrorResponse(c, http.StatusBadRequest, "Invalid request body", err, nil)
	}
	...
	if err != nil {
		if errors.Is(err, domain.ErrUserAlreadyExists) {
			...
			return h.sendErrorResponse(c, http.StatusConflict, err.Error(), nil, nil)
		}
		utils.Logger.Error("CreateUser: Failed to create user in usecase", zap.Error(err))
		return h.sendErrorResponse(c, http.StatusInternalServerError, "Failed to create user", err, nil)
	}

	utils.Logger.Info("User created successfully", zap.String("user_id", user.ID.Hex()), zap.String("email", user.Email))
	return h.sendSuccessResponse(c, http.StatusCreated, toUserResponse(user), 1)
}
...
func (h *UserHandler) GetUserByID(c *fiber.Ctx) error {
	...
	if id == "" {
		...
		return h.sendErrorResponse(c, http.StatusBadRequest, "User ID is required", nil, nil)
	}
	if _, err := primitive.ObjectIDFromHex(id); err != nil {
		utils.Logger.Warn("GetUserByID: Invalid user ID format", zap.String("id", id), zap.Error(err))
		return h.sendErrorResponse(c, http.StatusBadRequest, "Invalid user ID format", err, nil)
	}
	...
	if err != nil {
		if errors.Is(err, domain.ErrUserNotFound) {
			...
			return h.sendErrorResponse(c, http.StatusNotFound, err.Error(), nil, nil)
		}
		utils.Logger.Error("GetUserByID: Usecase error", zap.String("user_id", id), zap.Error(err))
		return h.sendErrorResponse(c, http.StatusInternalServerError, "Failed to retrieve user", err, nil)
	}

	utils.Logger.Info("User retrieved successfully", zap.String("user_id", user.ID.Hex()))
	return h.sendSuccessResponse(c, http.StatusOK, toUserResponse(user), 1)
}
...
func (h *UserHandler) GetAllUsers(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		return h.sendErrorResponse(c, http.StatusInternalServerError, "Failed to retrieve users", err, nil)
	}
	...
	return h.sendSuccessResponse(c, http.StatusOK, userResponses, len(userResponses))
}
...
func (h *UserHandler) UpdateUser(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		if errors.Is(err, domain.ErrUserAlreadyExists) { // เพิ่มการจัดการ error กรณี email ซ้ำ
		...
		return h.sendErrorResponse(c, http.StatusInternalServerError, "Failed to update user", err, nil)
	}

	utils.Logger.Info("User updated successfully", zap.String("user_id", updatedUser.ID.Hex()))
	return h.sendSuccessResponse(c, http.StatusOK, toUserResponse(updatedUser), 1)
}
...
func (h *UserHandler) DeleteUser(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		return h.sendErrorResponse(c, http.StatusInternalServerError, "Failed to delete user", err, nil)
	}
